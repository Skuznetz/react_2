        ...
        if (e.target.value.trim().length > 0){
            ...
            render: function() {
                return ( < form className= 'add-cf'>
                    <
                    input type='text' onChange={this.onAuthorChange} className = 'add-author'
                    defaultValue='' placeholder = "введите Ваше Имя" ref='author' />
                    <textarea onChange={this.onTextChange} className='add_text' defaultValue='' placeholder='Текст новости' ref='text'></textarea>
                    ...
                    <
                    button className='add_btn' onClick = { this.onBtnClickHandler } ref='alert_button' disabled={agreeNotChecked || authorIsEmpty || textIsEmpty} > Показать alert < /button> </form>);}
